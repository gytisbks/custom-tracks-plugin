<?php
/**
 * Template for producer settings shortcode
 */
...
if (!isset($settings) || empty($settings)) {
    if (defined('CTOS_DEBUG') && CTOS_DEBUG) {
        error_log('Settings not provided to shortcode template, initializing default settings');
    }
...
<div class="ctos-settings-wrapper">
    <h2><?php _e('Custom Track Orders Settings', 'custom-track-ordering-system'); ?></h2>
    <p class="ctos-settings-intro"><?php _e('Configure your custom track production services and pricing.', 'custom-track-ordering-system'); ?></p>
    
    <form id="ctos-producer-settings-form" class="ctos-settings-form">
        <?php wp_nonce_field('ctos-nonce', 'ctos_nonce'); ?>
        
        <div class="ctos-form-section">
            <h3><?php _e('Service Availability', 'custom-track-ordering-system'); ?></h3>
        ...
        <div class="ctos-form-section">
            <h3><?php _e('Your Music Style', 'custom-track-ordering-system'); ?></h3>
        ...
        <div class="ctos-form-section">
            <h3><?php _e('Pricing & Delivery', 'custom-track-ordering-system'); ?></h3>
        ...
        <div class="ctos-form-section">
            <h3><?php _e('Additional Services', 'custom-track-ordering-system'); ?></h3>
...
<script>
    jQuery(document).ready(function($) {
        console.log('Producer settings form initialized (shortcode)');
        ...
        $('#ctos-producer-settings-form').on('submit', function(e) {
            ...
            console.log('Submitting producer settings form (shortcode)');
            ...
            // Convert addons to correct format
            var addons = [];
            $('.ctos-addon-row').each(function(index) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        $('#ctos-settings-message').text('<?php _e('Error saving settings', 'custom-track-ordering-system'); ?>').css('color', 'red').show();
                    }
                },
