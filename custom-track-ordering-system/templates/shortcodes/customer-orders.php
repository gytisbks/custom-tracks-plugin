<?php
/**
 * Template for displaying a customer's track orders
 */
defined('ABSPATH') || exit;

// Make sure we have orders
if (!isset($orders) || empty($orders)) {
    echo '<p>' . __('You have not placed any custom track orders yet.', 'custom-track-ordering-system') . '</p>';
    return;
}
?>

<div class="ctos-orders-wrapper">
    <h2><?php _e('My Custom Track Orders', 'custom-track-ordering-system'); ?></h2>
    ...
    <table class="ctos-orders-table">
        ...
        <tbody>
            <?php foreach ($orders as $order) : 
                $producer = get_userdata($order->producer_id);
                $producer_name = $producer ? $producer->display_name : __('Unknown', 'custom-track-ordering-system');
